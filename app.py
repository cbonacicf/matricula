...
color_area[4] = '#5bb800'
color_region = [cm.tab20c(i) for i in range(20)]

colores = {
    'U. Estatales CRUCH': '#5983ff',
    'U. Privadas CRUCH': '#c0d0ff',
    'U. CRUCH': '#c0d0ff',
    'U. Privadas': '#FFA900',
    'U. Privadas No CUP': '#ffdb00',
    'U. Privadas CUP': '#FFA900',
    'U. Privadas CUP No Acreditadas': '#f06e14',
    'Matrícula total': '#78ec4a',
    'Matrícula primer año': '#c8fb09',
    'Mujeres': '#f8d3ff',
    'Hombres': '#ccecff',
    'No binarios': '#d0d0d0',
    'Pregrado': '#ccd7f2',
    ...
    'Doctorado': '#926beb',
    'Postítulo': '#5b97b6',
    'Administración y Comercio': color_area[0],
    'Agropecuaria': color_area[1],
    'Arte y Arquitectura': color_area[2],
    'Ciencias Básicas': color_area[3],
    'Ciencias Sociales': color_area[4],
    'Derecho': color_area[5],
    'Educación': color_area[6],
    'Humanidades': color_area[7],
    'Salud': color_area[8],
    'Tecnología': color_area[9],
    'Carreras STEM': '#7ec0ee',
    'Carreras No STEM': '#bfefff',
    'Arica y Parinacota': color_region[0],
    'Tarapacá': color_region[1],
    'Antofagasta': color_region[2],
    'Atacama': color_region[3],
    'Coquimbo': color_region[4],
    'Valparaíso': color_region[5],
    'Metropolitana': color_region[6],
    "Lib. B. O'Higgins": color_region[7],
    'Maule': color_region[8],
    'Ñuble': color_region[9],
    'Biobío': color_region[10],
    'La Araucanía': color_region[11],
    'Los Ríos': color_region[12],
    'Los Lagos': color_region[13],
...
#     tipo = pl.Enum(list(eval(f'map_{variable}.keys()')))
#     return (
#         matric
#         .filter(**criterio)
#         .collect()
...
criterio = dict(matricula=1, nivel=1)   # default: matricula total, pregrado

# def ser_schema(criterio, variable):
#     esquema = base_datos(criterio, variable).cast({variable: pl.Utf8}).schema
#     return {k: 'pl.'+str(v) for k, v in dict(esquema).items()}
...
def crea_figura(datos, tipo):
    ...
    ax.margins(x=0.01)
    ax.legend(ncols=3, frameon=False, bbox_to_anchor=(0.5, -0.05), loc='upper center', fontsize=10)
...
encabezado = html.Div(
    dbc.Row([
        ...
        dbc.Col(
            html.H1(['Información de Matrícula'], style={'textAlign': 'center'}),
            width=7
        ),
    ], align='center'),
    style={'marginTop': 15}
...
tuplas = [
    ...
    ('Matrícula', 'matricula', map_matricula, 1),
...
def crea_criterio(tipo, matricula, genero, nivel, region, area, stem):
    dic = dict(zip(variables, [tipo, matricula, genero, nivel, region, area, stem]))
    dic_retorna = {}
    for item in dic:
        if dic[item]:
...
def exporta_datos2(datos, trans, param):
    output = BytesIO()
...
#        .select([pl.col(pl.String)]+[str(i) for i in range(2010, 2026)])
    ...
    with xlsxwriter.Workbook(output) as workbook:
        ...
        worksheet.write(0, 0, 'Datos de Matrícula', titulo)
...
def modifica_seleccion(tipo, matricula, genero, nivel, region, area, stem, var_local, tipo_graf, trans, param):
    crt = crea_criterio(tipo, matricula, genero, nivel, region, area, stem)
...
def exporta_datos_excel(_, datos, trans, param):
    df = exporta_datos2(datos, trans, param)
    return dcc.send_bytes(df, 'datos_matricula.xlsx')


# ejecución de la aplicación
if __name__ == '__main__':
    app.run() # debug=True, port=8055) # prueba local
